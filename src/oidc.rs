...
impl State {
  ...
  async fn try_initialize_state(
    ...
    let issuer_url = IssuerUrl::new(args.oidc_issuer_url).context("failed to parse issuer URL")?;
    ...
    let client =
      CoreClient::from_provider_metadata(provider_metadata, client_id, Some(client_secret))
        .set_redirect_uri(
          RedirectUrl::new(base_url + "/login_oidc_redirect").context("Invalid redirect URL")?,
        );
