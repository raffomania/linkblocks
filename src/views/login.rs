...
fn username_field(errors: &FormErrors, value: &str) -> Element {
    fragment().with([
        label(class("mt-10 text-neutral-400").for_("credentials[username]")).with("Username"),
...
fn password_field(errors: &FormErrors) -> Element {
    fragment().with([
        label(class("mt-4 text-neutral-400").for_("credentials[password]")).with("Password"),
