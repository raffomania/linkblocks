...
pub fn view(
  ...
  layout::layout(
    fragment()
      ...
      .with(
        layout.authed_info.as_ref().and_then(|authed_info| {
...
fn list_item_bookmark(bookmark: &db::Bookmark) -> Element {
  fragment().with([
    a([
      class(
        "block overflow-hidden leading-8 text-orange-100 hover:text-orange-300 text-ellipsis \
         whitespace-nowrap",
      ),
      href(&bookmark.url),
    ])
...
fn list_item_list(inner_list: &db::ListWithLinks) -> Element {
  fragment().with([
    a([
      class(
        "block overflow-hidden font-semibold leading-8 hover:text-fuchsia-300 text-ellipsis \
         whitespace-nowrap",
      ),
